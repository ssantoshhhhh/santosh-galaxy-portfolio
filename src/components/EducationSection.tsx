...
const EducationCard = ({ edu, index }: { edu: any, index: number }) => {
  const reveal = useScrollReveal(edu.side === 'left' ? 'left' : 'right', index * 100);

  return (
    <div 
      ref={reveal.ref}
      className={`flex items-center mb-12 ${edu.side === 'left' ? 'md:flex-row' : 'md:flex-row-reverse'} flex-col md:mb-12 mb-8 ${reveal.className}`}
      style={reveal.style}
    >
      {/* Content */}
      <div className={`w-full md:w-5/12 ${edu.side === 'left' ? 'md:pr-8 md:text-right' : 'md:pl-8 md:text-left'} text-center md:text-left mb-4 md:mb-0`}>
        <div className="card-3d bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 hover:border-white/20 transition-all duration-500">
          ...
          <p className="text-gray-400 text-sm mb-3 whitespace-pre-line">{edu.description}</p>
        </div>
      </div>
